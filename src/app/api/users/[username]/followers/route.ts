...
type Row = { follower: PopulatedUser | null };

/** Readers who follow this user. */
export async function GET(
  ...
  const rows = await UserFollow.find({ following: target._id })
    .sort({ createdAt: -1 })
    .limit(200)
    .populate("follower", "username name image bio")
    .lean<Row[]>();
